...
class Algorithm:
    def __init__(self,
        ...
        self.algotext = algotext
        self.algofile = algofile
        self.choose_loader = choose_loader
        self.benchmark_returns = benchmark_returns

        self.sim = TradingAlgorithm(
            namespace=self.namespace,
            data_portal=self.data,
            ...
            sim_params=SimulationParameters(
                ...
                trading_calendar=self.trading_calendar,
                capital_base=copy.copy(self.capital_base),
            ...
            **{
            ...
            } if self.algotext is None else {
                'algo_filename': getattr(self.algofile, 'name', '<algorithm>'),
                'script': self.algotext,
            }
        )

    def reset(self):
        for symb, orders in self.sim.get_open_orders().items():
            for order in orders:
                self.sim.cancel_order(order)

        self.sim.blotter.orders.clear()
        self.sim.metrics_tracker = None

        self.sim.gen = self.sim.get_generator()
        pass
